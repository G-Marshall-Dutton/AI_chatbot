    ...
    def testClassifier(self,filename): # To work, change classify to return arrival_t
        with open(filename + '.csv', mode='a') as results:
            ...
            for row in testing_journeys.iterrows():
                ...
                if isAccurate(actual_s, prediction, 120):
            ...
            print(accuracyDISS,accuracySTWMRKT,accuracyIPSWICH,accuracyMANNGTR,accuracyCLCHSTR,accuracySTFD,accuracyLIVST," ",overallAccuracy)
...
if __name__ == "__main__":
    ...
    while(True):
        ...
        print(secondsToTime(
            sm.classifyInstance({"to":t, "from":f, "planned_dep_time":ptd, "delay_mins":delay})))
        print(secondsToTime(
            md.classifyInstance({"to":t, "from":f, "planned_dep_time":ptd, "delay_mins":delay})))
        print(secondsToTime(
